		...
		- ### DOMANDE
			- Perché fa questo test ???
				...
				- altra cosa che non capisco é a che server `minimo_tassato`, detto diversmente ci sono fasce che non sono di esubero che hanno un `minimo_tassato` ? #[[glossary/minimo tassato]]
					...
					- $$
					  \begin{cases}
					  TF_{min\_tax}(k) : \mathbb{N} \Rightarrow \mathbb{R}  & & & \textrm{trova minimo tassato da fascia} \\
					  TF_{calc}(k) = \max(k,TF_{min\_tax}(k)) & & & \textrm{calcola il massimo tra} \quad k & \textrm{e} & TF_{min\_tax}(k)
					  \end{cases} $$
