			...
			- #### Analisi degli sviluppi collettiva
				...
				- qualsiasi richiesta di variazione in itinere di un task che alteri in qualche modo la lista di attivitá determinate dall'analisi iniziale va necessariamente
					- rianalizzata per determinare quante siano le variazioni, di quale entitá in termini di effort si stia parlando, se facente parte o meno dell'ambito del task attuale #Process/SDC/issue-tracking/change-request e a quel punto :
						- nel caso in cui il task soffisfasse i requisiti per stare nello sprint attuale si dovrebbe creare un subtask o un task normale collegato con opportuna relazione caused by, blo
						  logseq.order-list-type:: number
							- sempre e comunque é meglio tracciare questi item come card separate #jira/improvements per poter poi fare qualche report
						- creare una ulteriore attivitá che verrá schedulata in altro sprint
						  logseq.order-list-type:: number
